...
public class LoginActivity extends AppCompatActivity {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    try {
                        if (result.getUser() != null) {
                            // 서버 응답의 user 필드에서 LoginResponse 객체 사용
                            LoginResponse loginResponse = result.getUser();

                            // 사용자 데이터 저장
                            saveUserData(loginResponse);
                        } else {
                            Log.e(TAG, "사용자 데이터가 null입니다.");
                        }
                    } catch (Exception e) {
                    ...
                    Log.d(TAG, "서버 응답: " + result.toString());
    ...
    private void saveUserData(LoginResponse loginResponse) {
        ...
        int admin = loginResponse.getAdmin(); // LoginResponse에서 admin 값 가져오기

        // 디버깅을 위한 로그 추가
        Log.d(TAG, "=== 사용자 정보 저장 시작 ===");
        Log.d(TAG, "서버에서 받은 관리자 상태: " + admin);
        ...
        // 관리자 상태 저장 (중요!)
        editor.putInt(KEY_USER_ADMIN_STATUS, admin);
        Log.d(TAG, "관리자 상태 저장: " + admin);
        ...
        Log.d(TAG, "=== 저장된 정보 확인 ===");
        Log.d(TAG, "isLoggedIn: " + checkPrefs.getBoolean(KEY_IS_LOGGED_IN, false));
        Log.d(TAG, "autoLogin: " + checkPrefs.getBoolean(KEY_AUTO_LOGIN, false));
        Log.d(TAG, "name: " + checkPrefs.getString(KEY_NAME, "없음"));
        Log.d(TAG, "email: " + checkPrefs.getString(KEY_EMAIL, "없음"));
        Log.d(TAG, "password: " + (checkPrefs.getString(KEY_PASSWORD, "").isEmpty() ? "저장 안됨" : "저장됨"));
        Log.d(TAG, "mbti: " + checkPrefs.getString(KEY_MBTI, "없음"));
        Log.d(TAG, "admin status: " + checkPrefs.getInt(KEY_USER_ADMIN_STATUS, -1));
        Log.d(TAG, "=== 저장 완료 ===");
    }
