...
public class ChatActivity extends AppCompatActivity implements
    ...
    private MessagePaginationManager paginationManager;    // 사용자 정보
    ...
    private void setupNetwork() {
        ...
        // 웹소켓 초기화는 동기화 완료 후 onSyncCompleted에서 수행
    }
    /**
     * 페이지네이션 매니저 초기화
     */
    ...
    private void loadStoredMessages() {
        messageRepository.loadMessagesForGroup(currentGroupId, new MessageRepository.MessageLoadCallback() {            @Override
            public void onMessagesLoaded(List<Message> messages) {
                runOnUiThread(() -> {
                    messageList.clear();
                    messageList.addAll(messages);
                    // 모든 아이템이 새로 추가되었음을 알림
                    messageAdapter.notifyItemRangeInserted(0, messages.size());

                    if (!messages.isEmpty()) {
                        uiManager.scrollToBottom(messageAdapter.getItemCount());
                    }

                    // 웹소켓 연결을 동기화 완료 후로 이동
                    // initWebSocket();
                });
            }
    ...
    public void onSyncCompleted(List<Message> newMessages) {
        Log.d(TAG, "onSyncCompleted 호출됨, 새 메시지 수: " + (newMessages != null ? newMessages.size() : 0));
        ...
        runOnUiThread(() -> {
            if (!newMessages.isEmpty()) {
                ...
                for (Message message : newMessages) {
                    ...
                    // 더 효율적인 알림: 추가된 메시지 범위만 알림
                    int startPosition = messageList.size() - addedCount;
                    messageAdapter.notifyItemRangeInserted(startPosition, addedCount);
                    // 메시지 동기화 후 강제로 최하단 스크롤
                    uiManager.forceScrollToBottom(messageAdapter.getItemCount());
                    ...
                    // 새 메시지가 없어도 초기 로드 시에는 스크롤 필요
                    if (messageList.size() > 0) {
                        uiManager.forceScrollToBottom(messageList.size());
                    }
                }
            } else {
                // 새 메시지가 없어도 초기 로드 시에는 스크롤 필요
            ...
            // 웹소켓 초기화 (아직 초기화되지 않았을 때만)
            if (!webSocketInitialized) {
                Log.d(TAG, "메시지 동기화 완료, 웹소켓 초기화 시작");
                initWebSocket();
                webSocketInitialized = true;
            } else {
                Log.d(TAG, "웹소켓이 이미 초기화됨");
            }
        });
    }
